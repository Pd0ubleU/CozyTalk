...
class AdminBanDetailScreen extends StatelessWidget {
  ...
  Widget _buildPageHeader(BuildContext context, bool hasCurrent, AdminBanDetailSubject s) {
    return Container(
        ...
        children: [
          GestureDetector(
            ...
            child: Container(
              ...
              child: const Icon(Icons.chevron_left_rounded, color: AdminC.brownDarker, size: 26),
            ),
          ),
          ...
          Column(
            ...
            children: [
              Text(
                ...
                style: const TextStyle(fontSize: 22, fontWeight: FontWeight.w800, color: Colors.white, letterSpacing: -.3),
              ),
              Text(
                hasCurrent
              ...
              Column(
                children: [
                  ...
                  Container(
                    ...
                    child: Text('UID: ${s.uid}', style: const TextStyle(fontSize: 10, color: AdminC.ink, fontWeight: FontWeight.w600)),
                  ),
                ],
              ),
                ...
                child: Column(
                  ...
                  children: [
                    Text(
                      ...
                      style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: AdminC.inkSoft, letterSpacing: 1),
                    ),
                    const SizedBox(height: 2),
                    Text(s.name, style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w800, color: AdminC.ink)),
                    const SizedBox(height: 10),
                    const Text('Interest', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: AdminC.ink)),
                    const SizedBox(height: 2),
                    Text(
                      s.interest.isNotEmpty ? s.interest : '—',
                      style: const TextStyle(fontSize: 12, color: AdminC.inkSoft, height: 1.4),
                    ),
                  ],
                ),
              ),
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                _statCell(s.joined.isNotEmpty ? _accountAge(s.joined) : '—', 'Account age', null),
                Container(width: 1, height: 30, color: AdminC.border),
                _statCell(
                  hasCurrent ? 'Banned' : (s.online ? 'Active' : 'Offline'),
                  'Status',
                  hasCurrent ? AdminC.red : (s.online ? AdminC.greenInk : AdminC.inkSoft),
                ),
              ],
            ),
          ),
  ...
  Widget _sectionLabel(String label) => Padding(
    ...
    child: Text(label, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w800, color: AdminC.ink)),
  );
  ...
  Widget _buildBanRecord(AdminBanRecord rec, {required bool current}) {
    return Container(
      decoration: BoxDecoration(
        color: Colors.white,
        borderRadius: BorderRadius.circular(16),
        border: current ? const Border(left: BorderSide(color: AdminC.red, width: 4)) : null,
            ...
            children: [
              if (current) ...[
                Container(
                  ...
                  child: const Text('CURRENT', style: TextStyle(color: Colors.white, fontSize: 9.5, fontWeight: FontWeight.w800, letterSpacing: .8)),
                ),
                const SizedBox(width: 6),
              ],
              Expanded(child: Text(rec.reason, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w800, color: AdminC.ink))),
              const SizedBox(width: 8),
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 9, vertical: 3),
                decoration: BoxDecoration(color: AdminC.brownDarker, borderRadius: BorderRadius.circular(999)),
                child: Text(rec.duration, style: const TextStyle(color: Color(0xFFFFF7E8), fontSize: 10.5, fontWeight: FontWeight.w800)),
              ),
            ],
          ),
          const SizedBox(height: 4),
          Text(rec.date, style: const TextStyle(fontSize: 11.5, color: AdminC.inkSoft)),
          if (rec.note.isNotEmpty) ...[
            const SizedBox(height: 10),
            Container(
              width: double.infinity,
              padding: const EdgeInsets.all(10),
              decoration: BoxDecoration(color: AdminC.creamDeep, borderRadius: BorderRadius.circular(10)),
              child: Text(rec.note, style: const TextStyle(fontSize: 12.5, color: AdminC.ink, height: 1.5)),
            ),
          ],
          const SizedBox(height: 10),
          Row(
            ...
            children: [
              Text.rich(TextSpan(children: [
                ...
                TextSpan(text: rec.by, style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: AdminC.ink)),
              ])),
              if (current && rec.expires != null && rec.expires != '—')
                Text.rich(TextSpan(children: [
                  const TextSpan(text: 'expires ', style: TextStyle(fontSize: 11, color: AdminC.inkSoft)),
                  TextSpan(text: rec.expires, style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: AdminC.ink)),
                ])),
            ],
          ),
        ],
  ...
  Widget _buildEmptyState() {
    return Container(
      ...
      child: Center(
        child: Column(
          children: [
            const Icon(Icons.shield_rounded, color: AdminC.brownDarker, size: 28),
            const SizedBox(height: 6),
            const Text('No ban history. Clean record ✨', style: TextStyle(color: AdminC.inkSoft, fontSize: 13)),
          ],
        ),
      ),
    );
    ...
    return Container(
      ...
      child: SizedBox(
        ...
        child: AdminModalBtn(
          ...
          bg: AdminC.green,
          fg: AdminC.greenInk,
          onTap: () => showAdminConfirmUnban(context: context, username: s.name, onConfirm: () { onUnban!(s); Navigator.pop(context); }),
        ),
      ),
    );
  }
